...
const sidebarItems: SidebarItem[] = [
    // Shared
    { label: "Dashboard", path: "/dashboard", icon: <LayoutDashboard className="w-5 h-5" />, roles: ["CANDIDATE", "EMPLOYER", "ADMIN"] },
    ...
    { label: "My Jobs", path: "/dashboard/manage-jobs", icon: <Briefcase className="w-5 h-5" />, roles: ["EMPLOYER"] },
    { label: "Applicants", path: "/dashboard/applicants", icon: <Users className="w-5 h-5" />, roles: ["EMPLOYER"] },
        ...
        <aside className="w-64 bg-[#2c3e50] text-white min-h-screen flex flex-col pt-8">
            <div className="px-6 mb-10">
                <Link to="/" className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-[#00b4d8] rounded flex items-center justify-center font-bold text-white">J</div>
                    <span className="text-xl font-bold tracking-tight">Job<span className="text-[#00b4d8]">Portal</span></span>
                </Link>
            </div>
