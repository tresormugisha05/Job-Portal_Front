...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "https://job-portal-back-fdlt.onrender.com/api";
...
export default function AdminEmployers() {
    ...
    const handleVerifyToggle = async (employerId: string, currentStatus: boolean) => {
        try {
            setVerifying(employerId);
            const token = localStorage.getItem("token");
            const response = await axios.patch(
                `${API_BASE_URL}/employers/${employerId}/verify`,
            ...
            if (response.data.success) {
                ...
                setEmployers((prev) =>
                    prev.map((emp) =>
                        emp._id === employerId ? { ...emp, isVerified: !currentStatus } : emp
                    )
                );
            }
        } catch (error) {
