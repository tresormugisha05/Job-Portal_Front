...
export default function EmployerListPage() {
  ...
  const [employers, setEmployers] = useState<EmployerWithOpenings[]>([]);
    ...
    const fetchEmployersWithJobs = async () => {
      try {
        ...
        if (employersData && employersData.length > 0) {
          // Filter only verified employers
          const verifiedEmployers = employersData.filter(
            (emp) => emp.isVerified === true,
          );

          const employersWithOpenings: EmployerWithOpenings[] =
            verifiedEmployers.map((emp) => {
              ...
              return {
                ...emp,
                id: emp.id || emp._id,
                openings: employerJobs.length,
              };
            });

          setEmployers(employersWithOpenings);
        } else {
          // Fallback to default data if API returns empty
          setEmployers(
            (defaultEmployers as any).map((emp: any) => ({
              ...emp,
              openings: emp.openings || 0,
            })),
          );
        }
      } catch (error) {
        console.error("Error fetching employers:", error);
        setApiError("Failed to load employers. Using default data.");
  ...
  const companyGroups = employers.reduce(
    (groups, company) => {
    ...
    {} as Record<string, EmployerWithOpenings[]>,
  );
                    ...
                    <div key={letter}>
                      ...
                      <ul className="space-y-4">
                        {companyGroups[letter].map((company) => (
                          ...
                          >
                            <div className="w-0 h-0 border-t-[6px] border-t-transparent border-l-10 border-l-orange-500 border-b-[6px] border-b-transparent shrink-0"></div>
                            <Link
                              to={`/employers/${company.id || company._id}`}
                              className="text-gray-700 font-medium group-hover:text-[#00b4d8] transition-colors text-base"
                            >
                              {company.companyName} ({company.openings})
                            </Link>
                          </li>
                        ))}
                      </ul>
