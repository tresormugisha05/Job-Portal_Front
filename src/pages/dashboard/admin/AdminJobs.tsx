...
import { Search, Filter, ShieldCheck, ShieldAlert, Trash2, Eye, MapPin, Building2 } from "lucide-react";
    ...
    return (
        <DashboardLayout>
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 mb-2">Manage All Jobs</h1>
                    <p className="text-gray-500 text-sm">Platform-wide job listing management and moderation.</p>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Posted By</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {jobs.map((job: any) => (
                                <tr key={job.id} className="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="text-sm font-medium text-gray-700">{job.postedBy}</p>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        {job.date}
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${job.statusColor}`}>
                                            {job.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button className="p-2 text-gray-400 hover:text-[#00b4d8] hover:bg-blue-50 rounded-lg transition-all" title="View">
                                                <Eye className="w-4 h-4" />
                                            </button>
                                            <button className="p-2 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all" title="Feature">
