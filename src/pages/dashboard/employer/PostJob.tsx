      ...
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fade-in">
          ...
          <p className="text-gray-500 max-w-md mb-8">
            Your listing for{" "}
            <span className="font-bold text-[#00b4d8]">{formData.title}</span>{" "}
            has been published.
          </p>
    ...
    <DashboardLayout>
      <div className="mb-10 text-left">
        ...
        <p className="text-gray-500">
          Provide all details to attract the best talent for your team.
        </p>
      </div>

      <form
        onSubmit={handleSubmit}
        className="max-w-5xl space-y-8 pb-20 text-left"
      >
        {error && (
        ...
        <DashboardSection title="Job Banner / Flyer">
          <p className="text-xs text-gray-400 uppercase font-bold tracking-widest mb-4">
            Promotional visual for the job page
          </p>
          {bannerPreview ? (
            <div className="relative rounded-2xl overflow-hidden border-2 border-[#00b4d8] group aspect-[24/10] bg-gray-100 shadow-lg">
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 backdrop-blur-sm">
                <button
                  ...
                  className="bg-white/20 hover:bg-white/40 text-white p-4 rounded-full transition-all shadow-xl"
                >
                ...
                <button
                  ...
                  className="bg-red-500/80 hover:bg-red-600 text-white p-4 rounded-full transition-all shadow-xl"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
            ...
            <div
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-gray-200 rounded-2xl p-16 text-center hover:border-[#00b4d8] hover:bg-blue-50/30 transition-all cursor-pointer group bg-white shadow-sm"
            >
              ...
              <p className="text-sm text-gray-500 mb-6">
                Visual listings get 3x more engagement. Recommended: 500x500px.
              </p>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-700 uppercase tracking-widest flex items-center gap-2">
                    <Briefcase className="w-3 h-3 text-[#00b4d8]" /> Position
                    Title
                  </label>
                  <input
                    required
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-700 uppercase tracking-widest flex items-center gap-2">
                    <Clock className="w-3 h-3 text-[#00b4d8]" /> Application
                    Deadline
                  </label>
                  <input
                    required
                ...
                <div className="space-y-4">
                  <label className="text-xs font-bold text-gray-700 uppercase tracking-widest flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-[#00b4d8] hover:text-[#009bc2] flex items-center gap-1 font-bold"
                    >
                    ...
                    {formData.responsibilities.map((item, idx) => (
                      <div key={idx} className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            removeListItem(idx, "responsibilities")
                          }
                  ...
                  <div className="space-y-3">
                    {formData.requirements.map((item, idx) => (
                      <div key={idx} className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => removeListItem(idx, "requirements")}
                          className="p-3 text-gray-300 hover:text-red-500 transition-colors"
                        >
                          <Trash2 className="w-5 h-5" />
                        </button>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 text-left">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {formData.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-[#00b4d8] text-xs font-bold rounded-lg border border-blue-100"
                  >
                    {tag}
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={addTag}
                  className="p-2 bg-[#00b4d8] text-white rounded-xl shadow-md hover:bg-[#009bc2]"
                >
