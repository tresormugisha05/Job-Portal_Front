...
export default function EmployerDetailPage() {
    ...
    useEffect(() => {
        ...
        if (selectedTypes.length > 0) {
            filtered = filtered.filter((job: any) => selectedTypes.includes(job.jobType || job.type || ''));
        }
    ...
    const categories = Array.from(new Set(employerJobs.map((job: any) => job.category)))
        .map(category => ({
            name: category,
            count: employerJobs.filter((job: any) => job.category === category).length
        }))
        .sort((a, b) => b.count - a.count);

    const locations = Array.from(new Set(employerJobs.map((job: any) => job.location)))
        .map(location => ({
            name: location,
            count: employerJobs.filter((job: any) => job.location === location).length
        }))
        .sort((a, b) => b.count - a.count);

    const jobTypes = Array.from(new Set(employerJobs.map((job: any) => job.jobType || job.type || "Full-time")))
        .map(type => ({
            name: type,
